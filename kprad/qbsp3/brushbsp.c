...
qboolean WindingIsHuge (winding_t *w)
{
	...
	{
		for (j=0 ; j<3 ; j++)
			if (w->p[i][j] < -max_bounds*2 || w->p[i][j] > max_bounds*2) // was 8000
				return true;
	}
	return false;
}
...
//			cs->original = s->original;
	...
	for (i=0 ; i<2 ; i++)
	{
		...
		for (j=0 ; j<3 ; j++)
		{
			if (b[i]->mins[j] < -max_bounds || b[i]->maxs[j] > max_bounds) // was -4096, 4096
			{
...
tree_t *BrushBSP (bspbrush_t *brushlist, vec3_t mins, vec3_t maxs)
{
	...
	// filter in optimized detail and terrain/gensurf brushes here

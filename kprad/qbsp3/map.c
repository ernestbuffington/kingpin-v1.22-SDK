...
#include "qbsp.h"
#include "ctype.h"
#include "outdim.h" // outdimensions
...
int	BrushContents (mapbrush_t *b)
{
	...
	if ( trans & (SURF_TRANS33|SURF_TRANS66|SURF_ALPHATEST) ) // Knightmare- added alphatest
	{
...
extern qboolean	verbosebrushes;		// Knightmare added
...
void ParseBrush (entity_t *mapent)
{
	...
	do
	{
		...
		// get content/surf flags & light value if available
		if (TokenAvailable())
		{
			GetToken (false);
		...
		// Knightmare- added surface color support
		for (i=0; i<3; i++)
			td.color[i] = 0;
		...
		if (TokenAvailable())
		{
			for (i=0; i<3; i++) {
				GetToken (false);
				td.color[i] = atoi(token);
			}
		}
		// end Knightmare

		// Knightmare- set fogplane surf flag for fog brushes
		if (side->contents & CONTENTS_FOG)
			side->surf |= SURF_FOGPLANE;
		...
		if (!(side->contents & ((LAST_VISIBLE_CONTENTS-1) 
			| CONTENTS_PLAYERCLIP|CONTENTS_MONSTERCLIP|CONTENTS_MIST|CONTENTS_FOG)  ) ) // Knightmare- added fog
		...
		side->texinfo = TexinfoForBrushTexture (&mapplanes[planenum],
			&td, vec3_origin, side->contents, b->isTerrain); // Knightmare added
...
qboolean	ParseMapEntity (void)
{
	...
	if (num_entities == max_entities) // Knightmare changed- was MAX_MAP_ENTITIES
		...
		for (i=0 ; i<mapent->numbrushes ; i++)
		{
			...
			for (j=0 ; j<b->numsides ; j++)
			{
				...
				s->texinfo = TexinfoForBrushTexture (&mapplanes[s->planenum],
					&side_brushtextures[s-brushsides], mapent->origin, s->contents, false); // Knightmare added
			}
...
{		
	...
	{
		if (mapbrushes[i].mins[0] > max_bounds) //was 4096

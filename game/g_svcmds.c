...
// KOOGLEBOT_ADD
	else if(Q_stricmp (cmd, "koogledebug") == 0)
	...
	else if (Q_stricmp (cmd, "addbot") == 0)
	{
		...
		{
			safe_bprintf (PRINT_MEDIUM, "KOOGLE: Can only spawn bots in deathmatch mode.\n");
			return;
		}
		if(ctf->value) // name, skin, team
			KOOGLESP_SpawnBot (gi.argv(2), gi.argv(3), gi.argv(4), NULL);
		else // name, skin
			KOOGLESP_SpawnBot (NULL, gi.argv(2), gi.argv(3), NULL);
	}	

	// removebot
    else if(Q_stricmp (cmd, "removebot") == 0)
    	KOOGLESP_RemoveBot(gi.argv(2));
	// Node saving

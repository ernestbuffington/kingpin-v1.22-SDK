...
void Cmd_Players_f (edict_t *ent)
{
	...
	for (i = 0 ; i < count ; i++)
	{
		Com_sprintf (q2small, sizeof(q2small), "%3i %s\n",
			game.clients[index[i]].ps.stats[STAT_FRAGS],
			game.clients[index[i]].pers.netname);
		if (strlen (q2small) + strlen(q2large) > sizeof(q2large) - 100 )
	...
	//	strncat (large, small);
		Q_strncatz (q2large, q2small, sizeof(q2large));
	}
...
void SetSensitivities (edict_t *ent, qboolean reset)
{
	...
	if (reset)
	{
#ifndef KINGPIN_ENGINE_MOD // engine has zoom autosensitivity
...
#ifndef KINGPIN_ENGINE_MOD // these functions moved clientside in engine

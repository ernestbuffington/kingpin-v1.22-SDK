...
void Blaster_Fire (edict_t *ent, vec3_t g_offset, int damage, qboolean q2hyper, int effect, int color)
{
	vec3_t	forward, right;
	vec3_t	start, offset;
	int		muzzleflash;
	...
	if (!q2hyper)
		fire_blaster (ent, start, forward, damage, sk_blaster_speed->value, effect, q2hyper, color);
	else
		fire_blaster (ent, start, forward, damage, sk_hyperblaster_speed->value, effect, q2hyper, color);

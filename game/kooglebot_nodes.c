...
///////////////////////////////////////////////////////////////////////
//
//  KOOGLE - Kingpin Bot Base Code
//
//  Version 1.0
//
//	This file is Copyright(c), The 86it Developer Network 2021, All 
//  Rights Reserved
//
//  The original file is Copyright(c), Steve Yeager 1998, All Rights 
//  Reserved
//
//
...
void KOOGLEND_SetGoal(edict_t *self, int goal_node)
{
	...
	node = KOOGLEND_FindClosestReachableNode(self, NODE_DENSITY*3, NODE_ALL);
...
void KOOGLEND_GrapFired(edict_t *self)
{
	...
	if(self->owner->client->ctf_grapplestate == CTF_GRAPPLE_STATE_PULL)
	{
		...
		closest_node = KOOGLEND_FindClosestReachableNode(self,NODE_DENSITY,NODE_GRAPPLE);
		if(closest_node == -1 ) // we need to drop a node
		{	
			closest_node = KOOGLEND_AddNode(self,NODE_GRAPPLE);
...
qboolean KOOGLEND_CheckForLadder(edict_t *self)
{
	...
	if (gi.pointcontents(self->s.origin) & CONTENTS_LADDER && self->velocity[2] > 0)
	{
		...
		closest_node = KOOGLEND_FindClosestReachableNode(self,NODE_DENSITY,NODE_LADDER); 
		if(closest_node == -1)
		{
			closest_node = KOOGLEND_AddNode(self,NODE_LADDER);
...
void KOOGLEND_PathMap(edict_t *self)
{
	...
	if(self->is_jumping)
	{
	   // See if there is a closeby jump landing node (prevent adding too many)
		closest_node = KOOGLEND_FindClosestReachableNode(self, 64, NODE_JUMP);

		if(closest_node == INVALID)
			closest_node = KOOGLEND_AddNode(self,NODE_JUMP);
	...
	closest_node = KOOGLEND_FindClosestReachableNode(self, NODE_DENSITY, NODE_ALL);
	...
	if(self->groundentity && self->groundentity->use == Use_Plat)
	{
	 ...
	 if(closest_node == INVALID)
	 {
		...
		if(self->waterlevel)
			closest_node = KOOGLEND_AddNode(self,NODE_WATER);
		else
		    closest_node = KOOGLEND_AddNode(self,NODE_MOVE);
...
{
	show_path_from = KOOGLEND_FindClosestReachableNode(self, NODE_DENSITY, NODE_ALL);
...
void KOOGLEND_LoadNodes(void)
{
    ...
    {
		// Create item table
		safe_bprintf(PRINT_MEDIUM, "KOOGLE: No node file found, creating new one...");
	...
	if(version == 1) 
	{
		safe_bprintf(PRINT_MEDIUM,"KOOGLE: Loading node table...");

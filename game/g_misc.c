...
//	sprintf (outfilename, "%s/%s", gamedir->string,DEADSOLDIER_MODEL);
	...
	// load original model
//	sprintf (infilename, "main/%s", DEADSOLDIER_MODEL);
	Com_sprintf (infilename, sizeof(infilename), "main/%s", DEADSOLDIER_MODEL);
	if ( !(infile = fopen (infilename, "rb")) )
	{
		...
		//	sprintf(pakfile,"%s/main/pak0.pak",cddir->string);
			Com_sprintf(pakfile, sizeof(pakfile), "%s/main/pak0.pak",cddir->string);
		...
		if(!data)
		{
			gi.dprintf("PatchDeadSoldier: Could not find %s in main/pak0.pak\n",DEADSOLDIER_MODEL);
			return 0;
		}
	}
	else

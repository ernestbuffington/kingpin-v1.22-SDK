...
{
#ifdef KINGPIN_ENGINE_MOD // use new engine file loading function instead
...
removeflags:
	...
	if (alias_data) { // If no alias file was loaded, don't bother
#ifdef KINGPIN_ENGINE_MOD // use new engine function instead
...
// KOOGLEBOT_ADD
	...
	if (deathmatch->value && strcmp(level.mapname,current_map))
	{
		...
		KOOGLEND_InitNodes();
		KOOGLEND_LoadNodes();
		//KOOGLESP_LoadBots();  // Knightmare- removed this
		KOOGLESP_LoadBotInfo(); // Knightmare- load bot info file

...
#ifdef KINGPIN_ENGINE_MOD // *.pak/pk3 support
	char	*file_data;
	int		file_size = 0;
	...
	for(i=game.maxclients+1; i<globals.num_edicts; i++) {
		...
		if(!s_match) {
			...
			for(j=0; j<NUM_ACTOR_SOUNDS; j++) {
				...
				// Search in main for external file
				Com_sprintf(filename, sizeof(filename), "%s/main/sound/%s%s",basedir->string,path,wavname[j]);
				f = fopen(filename,"r");
				if(f) {
					fclose(f);
					strncpy(filename, path, sizeof(filename));
				...
				// Search paks in main
				Com_sprintf(filename, sizeof(filename), "sound/%s%s",path,wavname[j]);
				if (InPak(basedir->string,"main",filename)) {
					strncpy(filename, path, sizeof(filename));
					strcat(filename, wavname[j]);
					e->actor_sound_index[j] = gi.soundindex(filename);
					continue;
				...
				if (strlen(cddir->string)) {
					...
					Com_sprintf(filename, sizeof(filename), "%s/main/sound/%s%s",cddir->string,path,wavname[j]);
					f = fopen(filename,"r");
					if(f) {
						fclose(f);
						strncpy(filename, path, sizeof(filename));
					...
					// Search paks in main
					Com_sprintf(filename, sizeof(filename), "sound/%s%s",path,wavname[j]);
					if (InPak(cddir->string,"main",filename)) {
						strncpy(filename, path, sizeof(filename));
						strcat(filename, wavname[j]);
						e->actor_sound_index[j] = gi.soundindex(filename);
						continue;
		...
		for(k=0; k<2; k++) {
			...
			// Search in main for external file
			Com_sprintf(path, sizeof(path), "%s/main/%s",basedir->string,filename);
			f = fopen(path,"r");
			if (f) {
				fclose(f);
				e->actor_model_index[k] = gi.modelindex(filename);
			...
			if (strlen(cddir->string)) {
				...
				Com_sprintf(path, sizeof(path), "%s/main/%s",cddir->string,filename);
				f = fopen(path,"r");
				if (f) {
					fclose(f);
					e->actor_model_index[k] = gi.modelindex(filename);

...
void KOOGLEAI_PickLongRangeGoal(edict_t *self)
{
	...
	// look for a target 
	current_node = KOOGLEND_FindClosestReachableNode(self,NODE_DENSITY,NODE_ALL);
	...
	for(i=0;i<num_players;i++)
	{
		...
		node = KOOGLEND_FindClosestReachableNode(players[i],NODE_DENSITY,NODE_ALL);
...
{	
	...
	if(KOOGLEIT_ChangeWeapon(self,FindItem("railgun")))
		return;
	...
	if(range > 300)
	{
		...
		// Knightmare added
		if (KOOGLEAI_CheckShot(self) && KOOGLEIT_ChangeWeapon(self,FindItem("homing rocket launcher")))
			return;
		if (KOOGLEAI_CheckShot(self) && KOOGLEIT_ChangeWeapon(self,FindItem("rocket launcher")))
			return;
	}
	...
	if(KOOGLEIT_ChangeWeapon(self,FindItem("blaster")))
   	   return;

...
//ZOID
	...
	bot->current_node = KOOGLEND_FindClosestReachableNode(bot,NODE_DENSITY, NODE_ALL);
...
void KOOGLESP_Respawn (edict_t *self)
{
	...
	if(ctf->value)
		KOOGLESP_PutClientInServer (self,true, self->client->resp.ctf_team);
	else
...
void KOOGLESP_LoadBotInfo()
{
	...
	{
		safe_bprintf(PRINT_MEDIUM,"KOOGLE: No bots.cfg file found, using default bots.\n");
		return; // bail
	}
	safe_bprintf(PRINT_MEDIUM,"KOOGLE: Loading bot data...");
...
void KOOGLESP_SpawnBot (char *team, char *name, char *skin, char *userinfo)
{
	...
	{	// Knightmare- rewrote this
		...
		KOOGLESP_PutClientInServer (bot,false, jointeam);
	}
	else
